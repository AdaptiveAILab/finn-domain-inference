...
def run_training(print_progress=True, model_number=None):
    ...
    for epoch in range(config.training.epochs):
        ...
        # start mini-batch learning
        for idx in np.arange(config.training.num_batches):
            print(f"idx --> {idx}")
            
            if train_mini_batch:
                u = th.tensor(np.load(os.path.join(data_path, f"sample_{str(idx).zfill(2)}.npy")),
                                         dtype=th.float).to(device=device)
                model.BC = th.tensor([[left_BC[idx]], [right_BC[idx]]])
                print(model.BC)
