...
"""
Main file for training a FINN model
"""
...
def run_training(print_progress=True, model_number=None):
    ...
    if config.data.type == "burger":
        ...
        # Add noise to data as decided in config
    ...
    if model.train_multi_batch:
        assert model.learn_BC is False,\
            "No possibility to learn BC in multi-batch scheme as the sequences have randomly initialized BCs."
        # Load boundary conditions of the multi-batches
        ...
        # Load t series for multi-batch training
        ...
        model.BC = BC
        # Print the shapes to check if everything worked well
        print(f"BC.shape -> {model.BC.shape}")
        ...
        if epoch_errors_train[-1] < best_train:
            ...
            if config.training.save_model:
                ...
                thread = Thread(target=helpers.save_model_to_file(
                    ...
                    bc=np.array2string(model.BC, precision=5),
                    net=model))
                thread.start()
