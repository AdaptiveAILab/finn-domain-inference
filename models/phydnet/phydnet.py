...
class EncoderRNN(torch.nn.Module):
    def __init__(self, phycell, convcell, input_channels, input_dim, _1d, bc, learn_BC,
                 device, retro_domain=None, sigmoid=True, small=True):
        """
        phycell: object
            The PhyCell object.
        convcell: object
        ...
        learn_BC: bool
            Whether to learn BC in training
        device: torch.device
            Device to perform calculation (CPU or CUDA).
        retro_domain: th.tensor[size of the spatial domain] or None
            Retrospective reconstruction domain. It is used when the aim is to infer unseen data
        sigmoid: bool
        ...
        if not self.learn_BC:
            ...
            self.bc = nn.Parameter(bc.clone().detach())
