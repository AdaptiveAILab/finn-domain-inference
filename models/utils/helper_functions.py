...
def save_model_to_file(model_src_path, config, epoch, epoch_errors_train,
                       epoch_errors_valid, bc, net, infer=False):
    """
    ...
    :param bc: the bc of the model
    :param net: The actual model
    :param infer: a boolean switch in case of inference
    :return: Nothing
    """

    if infer:
        model_save_path = os.path.join(
            model_src_path, "checkpoints", "infer_" + config.model.name
        )
    else:
        model_save_path = os.path.join(
    ...
    if infer:
        th.save(net.state_dict(),
                os.path.join(model_save_path, 'infer_' + config.model.name + ".pt"))
    else:
        th.save(net.state_dict(),
