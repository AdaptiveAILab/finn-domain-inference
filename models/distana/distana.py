...
class DISTANACell(nn.Module):
    ...
    def __init__(self, dyn_channels, lat_channels, hidden_channels, kernel_size,
        ...
        self.lat_in_conv_layer = conv_function(
            ...
            kernel_size=3,
            stride=1,
            padding=2, ### Changed to 2 in order to be able to feed BC's explicitly (1 in the original work)
            bias=bias
        ).to(device=device)
        ...
        self.pre_layer = conv_function(
            ...
            kernel_size=3,
            stride=1,
            padding=0, ### Changed to 0 in order to be able to feed BC's explicitly (1 in the original work)
            bias=bias
        ).to(device=device)
...
class DISTANA(nn.Module):
    ...
    """
    def __init__(self, config, device, bc, init_left=None, init_right=None,
                 retro_domain=None, learn_BC=False):
        """
        Constructor
        """
