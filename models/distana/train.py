...
def process_sequence(model, criterion, data, batch_size, epoch, config,
    ...
    for t in range(1, len(net_inputs)):
        ...
        closed_loop = True if epoch > 15 else False
...
def run_training(print_progress=False, model_number=None):
    ...
    device = "cpu" #helpers.determine_device(print_progress=False)
    ...
    if config.data.type == "burger":
        ...
        data = th.tensor(dset, dtype=th.float32).to(device=device)
    ...
    #
    # DISTANA -- Set up the training and validation datasets and -loaders
    data_train = th.clone(data[:config.training.t_stop]).unsqueeze(1).to(device=device)
    # th.tensor(data[:config.training.t_stop],device=device).unsqueeze(1)
    data_valid = th.clone(data[config.validation.t_start:config.validation.t_stop]).unsqueeze(1).to(device=device)
    # th.tensor(data[config.validation.t_start:config.validation.t_stop],device=device).unsqueeze(1)

    # # ConvLSTM -- Set up the training and validation datasets and -loaders
    # data_train = th.clone(data[:config.training.t_stop]).unsqueeze(1).unsqueeze(0).to(device=device)
    #     #th.tensor(data[:config.training.t_stop],device=device).unsqueeze(1)
    # data_valid = th.clone(data[config.validation.t_start:config.validation.t_stop]).unsqueeze(1).\
    ...
    for epoch in range(config.training.epochs):
        ...
        def closure():
            ...
            # # Train ConvLSTM
            # u_hat, _ = model(data_train)
            # mse = criterion(u_hat[0][0].squeeze(), data_train.squeeze())
        ...
        # Validate ConvLSTM
        # u_hat, _ = model(data_valid)
        # mse = criterion(u_hat[0][0].squeeze(), data_valid.squeeze())
