...
if TRAIN_DATA:
    ...
    T_MAX = 1
    T_STEPS = 201
    LEFT_BC = 6.0   #np.random.uniform(-1,1)
    RIGHT_BC = -6.0 #np.random.uniform(-1,1)
...
def write_data_to_file(root_path, simulator, sample, idx):
    ...
    if TRAIN_DATA:
        ...
        np.save(file=os.path.join(data_path, "t_series.npy"),
                arr=simulator.t[:len(simulator.t)]) #arr=simulator.t[:len(simulator.t)//2 + 1]
        ...
        if not RANGE_BC:
            ...
            np.save(file=os.path.join(data_path, f"sample_{str(idx).zfill(2)}.npy"),
                    arr=sample[:len(simulator.t)])     #arr=sample[:len(simulator.t)//2 + 1])
...
def create_data_tuple_colloc(simulator, sample):
    ...
    idx = np.random.choice(len(pair), DATAPOINTS_COLLOCATION , replace=True) #it was replace=False, Cosku changed it
