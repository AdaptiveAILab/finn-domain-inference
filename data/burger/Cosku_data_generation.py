...
if TRAIN_DATA:
    ...
    T_MAX = 1
    T_STEPS = 201
    LEFT_BC = 5.0    #np.random.uniform(-1,1)
    RIGHT_BC = -5.0  #np.random.uniform(-1,1)
...
def write_data_to_file(root_path, simulator, sample, idx):
    ...
    if TRAIN_DATA:
        ...
        if not RANGE_BC:
            ...
            np.save(file=os.path.join(data_path, f"sample{idx}.npy"),
                    arr=sample[:len(simulator.t)//2 + 1])
        ...
        np.save(file=os.path.join(data_path, "t_series.npy"), arr=simulator.t)
        np.save(file=os.path.join(data_path, "x_series.npy"), arr=simulator.x)
        np.save(file=os.path.join(data_path, f"sample.npy"), arr=sample)
